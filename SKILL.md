...
|---|---|---|
| [`nebu-pipeline-composer`](./skills/pipeline-composer/SKILL.md) | **stable** | Compose multi-stage Stellar ledger pipelines (origin → transform → sink) from existing processors. Discovers the catalog via `nebu list` and the flags of each stage via `--describe-json`. |
...
A skill that teaches the agent to always run describe first is durable across nebu releases. The `--describe-json` protocol is part of nebu's stable contract — see [`docs/STABILITY.md`](./docs/STABILITY.md).
